...
Message.validate = function(rawMessage, callback) {
  ...
  try {
    ...
    var id         = bitauth.getSinFromPublicKey(msg.pubkey);
    var time       = msg.message.split('::')[2] ? new Date(msg.message.split('::')[2]) : null;
    var collection = Storage.database.get(msg.message.split('::')[0]);
    var record     = collection.find({ id: id })[0];

    console.log(msg)

    if (msg.message.split('::')[0] !== 'id' && record) {
      // console.log(record, doc)
      if (time < new Date(record.timestamp)) {
