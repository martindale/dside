  ...
  peer.on('data', function(data) {
    ...
    Message.validate(data, function(valid) {
      if (valid) {
        ...
        return self.relay(peer, message);
      }
...
PeerManager.prototype.relay = function(peer, msg) {
  console.trace(msg)
  this.peers.forEach(function(p) {
    var pubkey       = msg.payload.pubkey;
    // unless it's an id msg, relay it
    var isVote       = msg.payload.message.split('::')[0] !== 'id';
    var isTrusted    = (config.trusted.indexOf(p.identity) !== -1);
    var peerIsSender = peer ? p.identity === peer.identity : false;
