...
var Server = function(port, client) {
  var self = this;

  self.port        = port;
  self.rpcPort     = port + 1;
  self.client      = (client instanceof Client) ? client : null;
  self._server     = network.createServer(self.handleConnection.bind(self));
  self._rpc        = http.createServer(self.handleRpcMessage());
  self._key        = fs.readFileSync(process.env.HOME + '/.dside/key').toString();
  self.connections = new PeerManager();
};
...
Server.prototype.handleConnection = function(connection) {
  ...
  Client.identify(self._key, self.connections.peers.map(function(p) {
    return p.connection;
  }));

  self.connections.sync(peer);
};
...
Server.prototype.handleRpcMessage = function(req, res) {
  ...
  handler.post('/', function(req, res) {
    ...
    try {
      Message.validate(payload, function(err) {
        if (err) throw new Error('!invalid message format');
        var msg = new Message().fromString(payload);

        log.info('relaying local rpc vote command to peers')

        if (self.client) {
          self.client.broadcast.bind(self.client)(msg);
        }

        self.connections.relay(null, msg);
        Message.handle({}, msg);
