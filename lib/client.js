  ...
  self.connections = self._nodes.map(function(n) {
    ...
    var client = network.createConnection(n.port, host, function() {
      ...
      if (self.ready()) {
        ...
        self.identify(self.connections);
...
Client.prototype.identify = function(connections) {
  var self     = this;
  var pubkey   = bitauth.getPublicKeyFromPrivateKey(self._key);
  var identity = bitauth.getSinFromPublicKey(pubkey);
  var str      = 'id::' + identity;
  var message  = new Message(str, bitauth.sign(str, self._key), pubkey);

  log.info('identifying self to peers');
